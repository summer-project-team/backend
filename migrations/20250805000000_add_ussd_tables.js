...
exports.up = function(knex) {
  return Promise.all([
    // USSD Sessions table
    knex.schema.createTable('ussd_sessions', function(table) {
    ...
    knex.schema.createTable('ussd_transactions', function(table) {
      table.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
    ...
    knex.schema.createTable('ussd_callbacks', function(table) {
      table.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
      table.uuid('session_id').notNullable().references('session_id').inTable('ussd_sessions').onDelete('CASCADE');
      table.string('network_code', 10).notNullable();
    ...
    knex.schema.createTable('ussd_analytics', function(table) {
      table.uuid('id').primary().defaultTo(knex.raw('gen_random_uuid()'));
