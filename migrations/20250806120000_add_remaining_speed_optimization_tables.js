...
exports.up = async function(knex) {
  ...
  const hasRateLocks = await knex.schema.hasTable('rate_locks');
  if (!hasRateLocks) {
  ...
  const hasParallelProcessingTasks = await knex.schema.hasTable('parallel_processing_tasks');
  if (!hasParallelProcessingTasks) {
  ...
  const hasInstantSettlementEligibility = await knex.schema.hasTable('instant_settlement_eligibility');
  if (!hasInstantSettlementEligibility) {
  ...
  const hasWebhookBatches = await knex.schema.hasTable('webhook_batches');
  if (!hasWebhookBatches) {
  ...
  const hasWebhookProcessingLogs = await knex.schema.hasTable('webhook_processing_logs');
  if (!hasWebhookProcessingLogs) {
